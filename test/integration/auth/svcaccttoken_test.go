...
func TestPeter(t *testing.T) {
	...
	createTokenRoleAndBinding(t, cs, sa.Name, "myns", "rolename2", []string{"webhookauthentication/apigroup"}, []string{"attest"}, []string{"authentication.engelbert.dev"})

	clusterRoles, err := cs.RbacV1().ClusterRoles().List(t.Context(), metav1.ListOptions{})
	if err != nil {
		t.Fatal(err)
	}
	foo := clusterRoles
